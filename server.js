...
// ==================== CÁC HÀM PHÂN TÍCH CẦU (giữ nguyên từ lần trước) ====================
function analyzeCauBet(results, type) { /* giữ nguyên code cũ */ 
...
function analyzeCauDao11(results, type) { /* giữ nguyên */ 
...
function analyzeCau22(results, type) { /* giữ nguyên */ 
...
function analyzeCau33(results, type) { /* giữ nguyên */ 
  if (results.length < 6) return { detected: false };
...
function analyzeCau121(results, type) { /* giữ nguyên */ 
  if (results.length < 4) return { detected: false };
...
function analyzeCau123(results, type) { /* giữ nguyên */ 
  if (results.length < 6) return { detected: false };
...
function analyzeCau321(results, type) { /* giữ nguyên */ 
  if (results.length < 6) return { detected: false };
...
function analyzeCauNhayCoc(results, type) { /* giữ nguyên */ 
  if (results.length < 6) return { detected: false };
  const skipPattern = [];
...
function analyzeCauNhipNghieng(results, type) { /* giữ nguyên */ 
...
function analyzeCau3Van1(results, type) { /* giữ nguyên */ 
  if (results.length < 4) return { detected: false };
...
function analyzeCauBeCau(results, type) { /* giữ nguyên */ 
  if (results.length < 5) return { detected: false };
  let streakLength = 1;
...
function analyzeCauTuNhien(results, type) { /* giữ nguyên */ 
...
function analyzeCau44(results, type) { /* giữ nguyên */ 
...
function analyzeCau55(results, type) { /* giữ nguyên */ 
...
function analyzeCau212(results, type) { /* giữ nguyên */ 
  if (results.length < 5) return { detected: false };
...
function analyzeCau1221(results, type) { /* giữ nguyên */ 
  if (results.length < 6) return { detected: false };
...
function analyzeCau2112(results, type) { /* giữ nguyên */ 
  if (results.length < 6) return { detected: false };
  const pattern = results.slice(0, 6);
...
function analyzeCauGap(results, type) { /* giữ nguyên */ 
  if (results.length < 6) return { detected: false };
...
function analyzeCauZiczac(results, type) { /* giữ nguyên */ 
  if (results.length < 8) return { detected: false };
...
function analyzeCauDoi(results, type) { /* giữ nguyên */ 
  if (results.length < 4) return { detected: false };
...
function analyzeCauRong(results, type) { /* giữ nguyên */ 
  if (results.length < 6) return { detected: false };
...
function analyzeSmartBet(results, type) { /* giữ nguyên */ 
  if (results.length < 10) return { detected: false };
...
app.get('/du-doan', async (req, res) => {
  try {
    ...
    await verifyPredictions('tele68', data.data);
...
app.get('/lich-su', async (req, res) => {
  try {
    const data = await fetchData();
    if (data && data.data) await verifyPredictions('tele68', data.data);
